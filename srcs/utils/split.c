...
/*   Updated: 2024/02/23 21:30:11 by tmekhzou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if ((command[sc->i] == '<' || command[sc->i] == '>') && command[sc->i + 1] == command[sc->i])
        handle_double_chars(command, sc);
    else if (command[sc->i] == '<' || command[sc->i] == '>' || command[sc->i] == '|')
...
char **ft_split_command(const char *command)
{
    ...
    sc.result_size = 0;
    sc.result_capacity = count_word_command(command) + 1;
